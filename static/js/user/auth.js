$(function () {
    ...
    let $username = $('#user_name'); //得到用户名
    let $mobile = $('#mobile'); //得到手机号

    let $smsCodeBtn = $('.form-item .sms-captcha');  // 获取短信验证码按钮
    let $imgCodeText = $('#input_captcha');  // 获取用户输入的图片验证码
    let $register = $('.form-contain');  // 获取注册表单元素

    generateImageCode();  // 打开网页自动生成图片验证码
    $img.click(generateImageCode);  // 当点击图片验证码生成新的图片验证码图片
        ...
        // 获取响应
            .done(function (res) {
                if (res.data.count !== 0) {
                    ...
                    message.showSuccess(res.data.username + "能够正常使用");
    ...
    function fn_check_mobile() {
        let sMobile = $mobile.val(); //获取手机号码输入框
        let SreturnValue = "";
        if (sMobile === "") {
        ...
        $.ajax({
            ...
            .done(function (res) {
                if (res.data.count !== 0) {
                    message.showError(res.data.mobile + '已经注册');
                    SreturnValue = ""
                } else {
